...
function init(onBlockedSite) {
  ...
  _pollSystem();
  _pollBrowserUrls();
  // Save timeline to disk every 60s instead of every 10s snapshot
...
function _pollBrowserUrls() {
  ...
  function scanTitles() {
    ...
    const ps = spawn('powershell', ['-NoProfile', '-NonInteractive', '-Command',
      "Get-Process | Where-Object {$_.MainWindowTitle -ne '' -and ($_.Name -match 'chrome|msedge|firefox|opera|brave')} | Select-Object -ExpandProperty MainWindowTitle"
    ], { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true });
    let out = '';
    ps.stdout.on('data', d => { out += d.toString(); });
    ps.on('close', () => {
      _scanRunning = false;
      out.split('\n').map(t=>t.trim()).filter(Boolean).forEach(t => _checkTitle(t));
    });
    ps.on('error', () => { _scanRunning = false; });
    setTimeout(() => { try { ps.kill(); } catch(_) {} _scanRunning = false; }, 3000);
  }
...
function _pollLogin() {
  ...
  ps.on('close', () => {
    try {
      const t = out.trim();
      if (/^\d{2}:\d{2}$/.test(t)) { _loginTime = t; log('ACTIVITY', 'Login: ' + t); }
    } catch(_) {}
  });
  ps.on('error', () => {});
  setTimeout(() => { try { ps.kill(); } catch(_) {} }, 10000);
}
...
let _lidPollRunning = false;
function _pollLidState() {
  if (process.platform !== 'win32' || _lidPollRunning) return;
  _lidPollRunning = true;
  const { spawn } = require('child_process');
  const ps = spawn('powershell', ['-NoProfile', '-NonInteractive', '-Command',
    "try{$m=Get-WmiObject -Namespace root/wmi -Class WmiMonitorBasicDisplayParams -EA Stop;Write-Output ($m.Count.ToString())}catch{Write-Output '1'}"
  ], { stdio: ['ignore', 'pipe', 'ignore'], windowsHide: true });
  let out = '';
  ps.stdout.on('data', d => { out += d.toString(); });
  ps.on('close', () => {
    _lidPollRunning = false;
    try {
      const monCount = parseInt(out.trim()) || 1;
      const newLidState = monCount === 0;
      if (newLidState && !_lastLidState) {
        _lidClosed = true; _lidCloseTime = Date.now(); _isSleeping = true;
        log('POWER', 'Lid closed / display off — session paused');
      } else if (!newLidState && _lastLidState) {
        _lidClosed = false; _isSleeping = false; _unlockTime = Date.now();
        log('POWER', 'Lid opened — resumed after ' + Math.round((Date.now()-_lidCloseTime)/60000) + 'm');
      }
      _lastLidState = newLidState;
    } catch(_) {}
  });
  ps.on('error', () => { _lidPollRunning = false; });
  setTimeout(() => { try { ps.kill(); } catch(_) {} _lidPollRunning = false; }, 4000);
}
...
function _pollSystem() {
  ...
  // Non-blocking async window/process poll using spawn
  const { spawn } = require('child_process');
  ...
  function poll() {
    ...
    ps.on('error', () => { _pollRunning = false; });
    setTimeout(() => { try { ps.kill(); } catch(_) {} _pollRunning = false; }, 4000);
  }
...
module.exports = { init, snapshot, getCurrentState, checkAlerts, daySummary, shiftSummary, getTimeline: () => _tl, getLastCamSnap, triggerCamCapture: _captureWebcam };
